...
type AdminTab = 'dashboard' | 'organisations' | 'participants' | 'programmes' | 'surveys' | 'impact' | 'analytics' | 'settings';
...
export const PlatformAdminDashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<AdminTab>(() => {
    ...
    if (path.includes('programmes')) return 'programmes';
  ...
  useEffect(() => {
    const path = location.pathname;
    if (path.includes('organisations')) setActiveTab('organisations');
    else if (path.includes('participants')) setActiveTab('participants');
    else if (path.includes('programmes')) setActiveTab('programmes');
    else if (path.includes('surveys')) setActiveTab('surveys');
    else if (path.includes('impact')) setActiveTab('impact');
    ...
    <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100">
        <div>
          ...
          <p className="text-xs text-slate-500">Review institutional registration documents before provisioning SaaS tenant workspaces.</p>
        </div>
        <div className="flex items-center gap-2">
          <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="text-xs border border-slate-200 rounded-lg px-3 py-1.5 bg-white text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 cursor-pointer">
          ...
          <tbody className="divide-y divide-slate-100">
            {filteredOrgs.map((org) => (
              <tr key={org.id} className="hover:bg-slate-50/70 transition-colors">
                ...
                <td className="py-3 px-3">
                  <span className="px-2 py-0.5 rounded-md bg-slate-100 font-medium text-[11px] text-slate-700 border border-slate-200">{org.organisation_type}</span>
                </td>
                <td className="py-3 px-3">
                ...
                <td className="py-3 px-3 text-right space-x-1.5 whitespace-nowrap">
                  {org.verification_status !== 'VERIFIED' && (
                    <button onClick={() => handleVerify(org.id, 'VERIFIED')} className="px-3 py-1.5 bg-slate-900 hover:bg-slate-800 text-white font-medium rounded-md text-xs transition-colors cursor-pointer shadow-xs">Approve & Verify</button>
                  )}
                  {org.verification_status !== 'REJECTED' && (
                    <button onClick={() => handleVerify(org.id, 'REJECTED')} className="px-3 py-1.5 border border-slate-200 text-slate-700 hover:bg-slate-100 font-medium rounded-md text-xs transition-colors cursor-pointer">Reject</button>
                  )}
                </td>
              </tr>
            ))}
  ...
  const renderContent = () => {
    switch (activeTab) {
      case 'organisations':
        return renderOrganisations();
      case 'participants':
        return renderPlaceholder('Participant Management', 'View and manage all participant profiles across organisations. Verify identities and track participation history.', Users);
      case 'programmes':
        return renderPlaceholder('Programme Oversight', 'Monitor all active programmes, track completion rates, and review programme-level outcomes across the network.', Building2);
      case 'surveys':
        return renderPlaceholder('Survey & Form Management', 'Review all surveys across organisations. Monitor response rates and data quality across baseline, monitoring, and endline forms.', ClipboardCheck);
      case 'impact':
        return renderPlaceholder('Impact Projects', 'Track cross-organisation impact projects, measure outcomes, and generate donor-ready reports.', TrendingUp);
      case 'analytics':
        return renderPlaceholder('Analytics & Reports', 'View aggregate analytics, export programme reports, and inspect outcome measurement across all tenants.', BarChart3);
      case 'settings':
        return renderPlaceholder('Platform Settings', 'Manage platform-wide settings, feature flags, and system configuration.', Settings2);
      default:
        return (
          <>
            {renderOrganisations()}
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1.5">
            <span className="px-2.5 py-0.5 rounded-md text-[10px] font-semibold bg-indigo-50 text-indigo-700 border border-indigo-100 uppercase tracking-wide">Platform Administration</span>
            <span className="text-xs text-slate-400">Voice of a Girl Global Network</span>
          </div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-900">System Governance & Organisation Verification</h1>
          <p className="text-xs text-slate-500 mt-1">Approve implementing partners, audit data integrity, and inspect aggregate impact telemetry.</p>
        </div>
      </div>
      ...
      {activeTab === 'dashboard' && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
            ...
            <div className="text-[11px] text-emerald-600 font-medium mt-1 flex items-center gap-1"><ArrowUpRight className="w-3.5 h-3.5" /> +24% Year over Year</div>
          </div>
          <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
